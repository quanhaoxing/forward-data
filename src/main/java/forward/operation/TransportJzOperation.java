...
public class TransportJzOperation {

    public static final Logger logger = Logger.getLogger(TransportJzOperation.class);

    public void forward() throws Exception {
        synchronized (LqData.transportJzList) {
            while (LqData.transportJzList.isEmpty()) {
                LqData.transportJzList.wait();
            }
            logger.info("----------------2.1 沥青运输(基质)缓存个数---------------->\t" + LqData.transportJzList.size());
            Entity entity = LqData.transportJzList.peek();
...
//            logger.info("-------------请求参数----->" + JSONUtil.parseObj(params).toString());
            logger.info("---------2.1 沥青运输(基质)接口返回结果----->" + result);
            ...
            if (result.contains("200")) {
                ...
                LqData.transportJzList.poll();
            }
            if (LqData.transportJzList.isEmpty()) {
                LqData.transportJzList.notifyAll();
            }
        }
        ...
        synchronized (LqData.transportJzList) {
            ...
            results = SqlUtil.select(
                    "SELECT\n" +
                            ...
                            "WHERE\n" +
                            "\t sport.Status in (0,1) and test.TestType = 0 and tag.TestType = 0 \n" +
                            "\t and sport.AsphaltFacId = 1 and af.Image5 = 1 \n" +
                            "\t order by sport.dataMakeTime desc ");
            logger.info("------------2.1 沥青运输(基质)获取数量----->\t" + (results == null? 0:results.size()));
            if (results != null && results.size() != 0) {
//                results.removeAll(transportList);
                LqData.transportJzList.addAll(results);
                LqData.transportJzList.notifyAll();
            }
